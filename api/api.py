...
from event.models import Event, ThemeOfEvent, SubscribeUser, EventComments
...
class EventCommentsAddAPIView(CreateAPIView):
	serializer_class = EventCommentsAddSerializer
	permission_classes = (IsAuthenticated,)
	...
	def post(self, request, *args, **kwargs):
		...
		comment = EventComments.objects.filter(user_id=request.user.id).exists()

		if comment:
			return Response({"message": "Вы уже оставили отзыв!"}, status.HTTP_400_BAD_REQUEST)
...
class EventCommentsDeleteAPIView(DestroyAPIView):
	serializer_class = EventCommentsRemoveSerializer
	permission_classes = (IsAuthenticated,)

	def delete(self, request, *args, **kwargs):
		data = request.data
		serializers = self.serializer_class(data=data, many=False)
		serializers.is_valid(raise_exception=True)

		event_comment = get_object_or_404(EventComments, user_id=request.user.id, event_id=data['event'], id=data['id'])
		event_comment.delete()

		return Response({"message": "Успешно"}, status.HTTP_200_OK)


class EventRatingAPIView(CreateAPIView):
	...
	def get(self, request):
		...
		total_user = SubscribeUser.objects.filter(event_id=data['event']).count()
		like = EventComments.objects.filter(like=True).count()
		comments = EventComments.objects.filter(~Q(comment=None), ~Q(comment='')).count()
		print(like, 'like')
		print(total_user, 'total_user')
		print(comments, 'comments')

		res = (like + comments) / total_user + 100
		print(res)
